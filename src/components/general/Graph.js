...
const Graph = (props) => {
    ...
    if (isNaN(R)) {
        R = Cookies.get("R-value")
    }

    function getXFromSVG(x, r) {
        return ((x - 150) / 60 / 2 * r) - 0.06 * r;
    }

    function getYFromSVG(y, r) {
        let svg = document.getElementById("svg2")
        let offset = svg.offsetTop - 300
        return ((y - 150) / 60 / 2 * -r) + offset / 100 * 0.81 * r;
    }
    ...
    function drawPointsFromTable() {
        $("tbody tr").each(function () {
            ...
            let Rval = Cookies.get("R-value")
            let redraw = Cookies.get("redrawDots")
            ...
            function xValueForPoint(x) {
                return (x / Rval * 2 * 60 + 150)
            }

            function yValueForPoint(y) {
                return (-y / Rval * 2 * 60 + 150)
            }
            ...
            let textPlot
            if (redraw === "true") {
                plot.html(plot.html().replaceAll("text"))
                existingPlot = plot.html().replaceAll("circle")
                Cookies.set("redrawDots", "false")

            } else {
                existingPlot = plot.html()
             }


            let text1=`<text x="265" y="140">${Rval}</text>`
            let text2=`<text x="200" y="140">${Rval / 2}</text>`
            let text3=`<text x="75" y="140">${-Rval / 2}</text>`
            let text4=`<text x="20" y="140">${-Rval}</text>`
            let text5=`<text x="156" y="35">${Rval}</text>`
            let text6=`<text x="156" y="95">${Rval / 2}</text>`
            let text7=`<text x="156" y="215">${-Rval / 2}</text>`
            let text8=`<text x="156" y="275">${-Rval}</text>`
            ...
            plot.html(existingPlot + newPlot+text1+text2+text3+text4+text5+text6+text7+text8)
        })
    }

    setInterval(drawPointsFromTable, 600)
